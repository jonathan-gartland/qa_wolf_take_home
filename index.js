...
async function buildArticleList(page, list) {
  ...
  list.push(
    ...articles.slice(
      0,
      Math.min(ARTICLES_PER_PAGE, MAX_ARTICLES - list.length),
    ),
  );
}
...
async function sortHackerNewsArticles() {
  ...
  assert(
    ...
    "Number of articles retrieved is not 100",
  );
  ...
  articles.sort(
    (a, b) =>
      new Date(b.datestring.split(" ")[0]) -
      new Date(a.datestring.split(" ")[0]),
  );
  ...
  datesInOrder === true
    ? console.log(
        `Articles are in descending date order newer -> older: ${datesInOrder}`,
      )
